...
const ProductPage = () => {
    ...
    const addToCart = ()=>{
        ...
        axios.post("http://localhost:5000/product/addtocart",{
            ...
            token:user.phone
        }).then((res)=>res.data==="not login"?navigate("/login"):successToast("Product is  added to cart")).catch((err)=>console.log(err));
    }
    const addWishlist = async()=>{
        ...
        await axios.post("http://localhost:5000/product/wishlist",{
            product:productDetails,
            token:user.phone
        }).then((res)=>res.data==="not login"?navigate("/login"):successToast("Product is  added to wishlist")).catch((err)=>console.log(err));
    }
    return (
        <>
